...
struct InParallelOpToSCFRewriter
  ...
  LogicalResult matchAndRewrite(linalg_ext::InParallelOp inParallelOp,
    ...
    scf::ForOp forOp = rewriter.create<scf::ForOp>(
        loc, zero, numThreads, one, performConcurrentlyOp.yieldedValues());

    rewriter.setInsertionPointToStart(forOp.getBody());

    BlockAndValueMapping bvm;
    bvm.map(performConcurrentlyOp.yieldedValues(), forOp.getRegionIterArgs());
    ...
    for (Value toReplace : performConcurrentlyOp.yieldedValues()) {
